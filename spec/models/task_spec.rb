...
describe Task, type: :model do
  describe 'instance methods' do
    describe '#laundry?' do
      ...
      it 'it returns false when neither the description nor title is laundry' do
        task = Task.create!(title: 'clean my clothes', description: 'again')

        expect(task.laundry?).to be(false)
      end

      it 'returns true when the title contains the word laundry' do
        task = Task.create!(title: 'clean my laundry', description: 'again')

        expect(task.laundry?).to be(true)
      end

      it 'is case insensitive when checking if the title contains the word laundry' do
        task = Task.create!(title: 'Laundry', description: 'again')
